...
public class Seeder implements ApplicationListener<ContextRefreshedEvent> {
  ...
  public void onApplicationEvent(final ContextRefreshedEvent event) {
    ...
    // == create initial user
    createUserIfNotFound("test@test.com", "Test", "Test", "adminokto", "00000000O", "000000000",
  ...
  void createUserIfNotFound(final String email, final String firstName, final String lastName, final String password,
      final String dni, final String phone, final Collection<Role> roles) {
    User user = userRepository.findByEmail(email);
    if (user == null) {
      user = new User(firstName, lastName, dni, email, passwordEncoder.encode(password), phone, roles);
